...
const EditAlbumForm = ({ setShowModal }) => {
  ...
  const handleOnSubmit = async (e) => {
    ...
    await dispatch(editAlbumThunk(editingAlbum))
      .then(() =>
        history.push(`/users/${userId}/albums/${editAlbum.id}/images`)
      )
      .catch(async (res) => {
    ...
    <div className="edit-album-container">
      ...
      <div className="edit-album-form-and-buttons">
        ...
        <div className="edit-album-form-buttons">
          <button
            className="edit-album-delete-btn"
            onClick={(e) => deleteHandler(e, editAlbum)}
          >
            Delete album
          </button>
          <button
            disabled={!title || description.length > 255}
            id="edit-album-form-button"
            type="submit"
