      ...
      <div className="others-album-header">
        ...
        <div className="others-album-header-bottom">
          <div className="others-album-header-left">
            <img
              alt=""
              src={require("../../images/torch.png")}
              className="others-album-header-pfp"
            />
            <div className="others-album-header-user-info">
              <div className="others-album-header-username">Survivr</div>
              <div className="others-album-header-fluff">
                {usersArr.length} users in the Survivr community
              </div>
            </div>
          </div>
          <div className="others-album-header-right">
            <div className="others-album-header-right-top"></div>
            <p className="others-album-header-photo-count">
        ...
        {othersAlbums.map((album) => {
          return (
            <div key={album.id} className="others-album-wrap">
              <Link
                className="others-album-pics-link"
                to={`/users/${user.id}/albums/${album.id}/images`}
              >
                <div className="others-album-pics">
                  <img
                    className="others-album-pics-one"
                    alt=""
                    title={
                      images.filter((image) => image.albumId === album.id)[0]
                        ? images.filter(
                    ...
                    src={
                      !images.filter((image) => image.albumId === album.id)[0]
                        ? "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQ7-PDfavcidh7T4Ocucr_DtQ-Xe_0MA3JQ5h5j93zs2j_S_IT65S1C58fbJsieXcKSGDE&usqp=CAU"
                        : images.filter(
                            (image) => image.albumId === album.id
                          )[0].imageURL
                    }
                  ...
                  <div className="others-album-pics-right">
                    <img
                      className="others-album-pics-two"
                      alt=""
                      title={
                        images.filter((image) => image.albumId === album.id)[1]
                          ? images.filter(
                              (image) => image.albumId === album.id
                            )[1].title
                          : null
                    ...
                    />
                    <img
                      className="others-album-pics-three"
                      alt=""
                      title={
                        images.filter((image) => image.albumId === album.id)[2]
                          ? images.filter(
                              (image) => image.albumId === album.id
                            )[2].title
                          : null
              ...
              <div className="others-album-info">
                <div className="others-album-title-container">
                  <Link
                    className="others-album-title-link"
                    to={`/users/${user.id}/albums/${album.id}/images`}
                  >
                    <span className="others-album-title">{album.title}</span>
                  </Link>
                </div>
                <div className="others-album-img-count">
                  {images.filter((image) => image.albumId === album.id).length}{" "}
                  items
                </div>
              </div>
            </div>
